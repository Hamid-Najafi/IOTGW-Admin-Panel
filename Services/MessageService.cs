...
namespace IOTGW_Admin_Panel.Services
{
    public interface IMessageService
    {
        ...
        Message NodeClaimCheck(Message messageParam);
    }
    ...
    public class MessageService : IMessageService
    {
        ...
        public Message NodeClaimCheck(Message messageParam)
        {
            var nodeId = messageParam.NodeId;
            var nodeId = _context.Messages.Include(m => m.Node).ThenInclude(n => n.Gateway).FirstOrDefault(m => m.Id == id);
            if (message == null)
                throw new AppException("Message not found");
            return message;
        }
