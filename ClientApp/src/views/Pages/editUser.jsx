...
class editUser extends Component {
    ...
    componentDidMount() {
        // this.setState({editId:this.props.location.state.id})
        let token  = reactLocalStorage.getObject('userInfo').token;
        if (token== null) {
            this.props.history.push('/login')
        }
        var targetId =0;
        if (this.props.location.state === undefined) this.props.history.push('/users/all')
        else {
            targetId =this.props.location.state.id;

            fetch("https://localhost:5001/api/Users/" + targetId , {
                method: 'GET',
                headers: {
